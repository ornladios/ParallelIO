            ...
            *varidp = file->num_vars;
			...
			/* TAHSIN -- some codes moved from pio_darray.c */
			{
        		...
        		if (file->adios_iomaster == MPI_ROOT)
        		{
					...
					if (av->ndims==0) { /* If zero dimensions, use adios_string instead of adios_string_array */
						char sdata[2]; sdata[0] = '0'; sdata[1] = '\0'; 
						adios_define_attribute_byvalue(file->adios_group,"__pio__/dims",av->name,adios_string,1,sdata);
					} else {
